...
public class TeamResource {
    ...
    /**
     * POST  /create-team : Create a new team for the CURRENT user.
     * (get by current JWT)
     *
     * @return the ResponseEntity with status 201 (Created) and with body the new teamDTO, or with status 400 (Bad Request) if the team has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
    ...
    @Timed
    public ResponseEntity<CreateTeamDTO> createTeam(@RequestBody CreateTeamDTO teamDTO) throws URISyntaxException {
        log.debug("REST request to save Team : {}", teamDTO);
        if (teamDTO.getId() != null) {
            throw new BadRequestAlertException("A new team cannot already have an ID", ENTITY_NAME, "idexists");
        }
        CreateTeamDTO result = teamService.create(teamDTO);
        return ResponseEntity.created(new URI("/api/teams/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, result.getId().toString()))
            .body(result);
    }
    ...
    @Timed
    public ResponseEntity<List<TeamDTO>> searchTeams(@RequestParam String query, @ApiParam Pageable pageable) {
