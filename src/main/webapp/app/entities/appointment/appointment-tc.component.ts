...
@Component({
    selector: 'jhi-appointment-tc',
    templateUrl: './appointment-tc.component.html'
})
export class AppointmentTcComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/appointment-tc'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/appointment-tc', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: AppointmentTc) {
        return item.id;
    }
