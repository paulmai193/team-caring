...
@Component({
    selector: 'jhi-custom-user-tc',
    templateUrl: './custom-user-tc.component.html'
})
export class CustomUserTcComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/custom-user-tc'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/custom-user-tc', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: CustomUserTc) {
        return item.id;
    }
